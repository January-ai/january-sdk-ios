...
if (serversStart < 0) throw new Error("OpenAPI servers block was not found.");
const serversContentStart = serversStart + "servers:\n".length;
const serversRemainder = source.slice(serversContentStart);
const nextTopLevelKey = serversRemainder.search(/^[A-Za-z][A-Za-z0-9_-]*:\n/m);
const serversEnd = nextTopLevelKey < 0 ? source.length : serversContentStart + nextTopLevelKey;
const servers = source.slice(serversContentStart, serversEnd).trimEnd().split(/(?=^  - url:)/m);
const productionServer = servers[0]?.trimEnd();
if (!productionServer?.startsWith("  - url:")) throw new Error("Production server was not found.");
source = source.slice(0, serversStart) + "servers:\n" + productionServer + "\n" + source.slice(serversEnd);
